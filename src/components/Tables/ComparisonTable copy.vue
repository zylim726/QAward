      ...
      <table class="nested-table">
        ...
        <tr>
          ...
          <template v-if="!isHide">
              <th scope="col"  v-for="(unitdata, index) in Unittype" :key="index">{{ unitdata.type }}</th>
          </template>
          <th scope="col">QTY</th>
...
export default {
  ...
  methods: {
    async getUnittype(id) {
      console.log('ID:', id);
      try {
        console.log('Fetching unit types for ID:', id);
        const response = await CallofQuotationController.getUnittype(id);
  ...
  mounted() {
     console.log("cqId received in comparisonTable:", this.unitId);
    const checkboxes = document.querySelectorAll('input[name="checkbox"]');
    checkboxes.forEach((checkbox) => {
      checkbox.addEventListener("change", () => {
        if (checkbox.checked) {
